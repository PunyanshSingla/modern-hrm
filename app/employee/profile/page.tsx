...
interface EmployeeProfile {
    firstName: string;
    lastName: string;
    email: string; // From auth, might need to fetch separately or attach to profile response
    phone: string;
    address: string;
    position: string;
                    ...
                    <div className="space-y-4">
                         <div className="flex items-center gap-4 group/item">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="font-bold">{profile.email || "Not provided"}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 group/item">
                            <div className="h-10 w-10 rounded-full bg-background border border-border flex items-center justify-center text-muted-foreground group-hover/item:text-primary transition-colors">
                        ...
                        <div className="flex items-center gap-4 group/item">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="font-bold italic">{format(new Date(profile.createdAt), "MMMM d, yyyy")}</span>
                            </div>
                        </div>
                    </div>
                </CardContent>
                            ...
                            {profile.certifications.map((cert, index) => (
                                <div key={index} className="p-4 rounded-2xl bg-background/50 border border-border/50 shadow-sm space-y-2">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] font-black uppercase text-muted-foreground pt-1">
                                       <span className="bg-muted w-fit px-2 py-0.5 rounded-full italic">{format(new Date(cert.date), "MMM yyyy")}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
